          ...
          <template v-if="agentFilter === 'pending' && !loadingAgents">
            ...
            <div v-for="app in pendingAgents" :key="app.id" class="agent-card">
              ...
              <div class="agent-card-body">
                ...
                <div v-if="app.ai_verification_result" class="ai-section">
                  <label>AI Verification Result</label>
                  <pre class="ai-result">{{ formatAiResult(app.ai_verification_result) }}</pre>
                </div>

                <!-- PRC License Photo -->
                <div v-if="app.prc_license_photo" class="prc-section">
  ...
  methods: {
    ...
    async loadOverview() {
      ...
      try {
        ...
        if (pendingTab && this.overview.pending_agents > 0) {
          pendingTab.badge = this.overview.pending_agents;
        }
    ...
    async approveAgent(app) {
      if (!confirm(`Approve ${app.applicant_name || app.user?.name} as an agent?`)) return;
      try {
    ...
    async confirmDelete() {
      ...
      try {
        ...
        if (this.deleteType === 'user') {
          ...
          alert(`${this.deleteTarget.name} has been deleted.`);
          await this.loadUsers(this.usersPagination.current_page);
        } else if (this.deleteType === 'property') {
          await adminAPI.deleteProperty(this.deleteTarget.id);
          alert(`${this.deleteTarget.title} has been deleted.`);
    ...
    async confirmFlagReview() {
      try {
        ...
        alert(this.flagReviewAction === 'approve' ? 'Flag cleared successfully!' : 'Action taken successfully!');
        this.showFlagReviewModal = false;
        await this.loadFlaggedProperties(this.flaggedPagination.current_page);
      } catch (error) {
        alert('Failed to review flag: ' + (error.response?.data?.message || error.message));
      } finally {
        this.processingId = null;
      }
    },
...
.dot-verify { background: var(--palace-gold); }
...
.agent-info { flex: 1; }
.agent-info h3 { font-size: 15px; font-weight: 600; color: var(--smoky-black); }
...
.prc-photo:hover { transform: scale(1.02); }
...
.close-btn:hover { color: var(--smoky-black); }
...
.chart-title { font-size: 14px; font-weight: 600; color: var(--smoky-black); margin-bottom: 12px; }
.bar-chart {
  display: flex;
...
.bar-fill-users { background: var(--palace-gold); }
.bar-fill-properties { background: var(--smoky-black); }
...
.rank-num { font-weight: 700; color: var(--palace-gold); width: 20px; }
.rank-name { flex: 1; color: var(--smoky-black); font-weight: 500; }
.rank-bar-wrap { width: 80px; height: 8px; background: var(--light-gray); border-radius: 4px; }
.rank-bar { height: 100%; background: var(--palace-gold); border-radius: 4px; transition: width 0.3s; }
...
.flag-info { flex: 1; }
.flag-info h3 { font-size: 15px; font-weight: 600; color: var(--smoky-black); }
...
.flag-reason-box p { font-size: 14px; color: var(--smoky-black); }
...
.flag-details li { font-size: 13px; color: var(--smoky-black); margin-bottom: 4px; }
...
.btn-create-admin:hover { background: var(--palace-gold-dark); transform: translateY(-1px); }
...
.text-input:focus { outline: none; border-color: var(--palace-gold); }
